...
describe User do
  ...
  describe "reseting password" do
    ...
    before(:each) do
      ...
      UserNotifier.stub!(:deliver_password_reset_instructions)
    end
    
    it "should reset temporary token" do
      @user.should_receive( :reset_perishable_token! )
      @user.deliver_password_reset_instructions!
    end
    
    it "should tell the UserNotifier to send password reset instructions" do
      UserNotifier.should_receive(:deliver_password_reset_instructions).with(@user)
      @user.deliver_password_reset_instructions!
    end
    
  end
  
  describe "returning its name" do

    it "should return first_name if first_name is preset" do
      user = User.create( valid_user_attributes )      
      user.name.should ==(valid_user_attributes[:first_name])
    end
    
    it "should return screen_name if first_name is NOT preset" do
      user = User.create( valid_user_attributes.except(:first_name) )      
      user.name.should ==(valid_user_attributes[:screen_name])
    end
